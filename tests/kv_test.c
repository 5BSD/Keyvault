 ...
 *
 * Verifies our Ed25519 implementation against the official test vectors.
 */
static int
test_ed25519_rfc8032_vector1(void)
{
	...
	static const unsigned char expected_pubkey[32] = {
	...
	TEST_START("Ed25519 RFC 8032 Test Vector 1");
	...
	/* Sign empty message */
	memset(&signreq, 0, sizeof(signreq));
	signreq.key_id = importreq.key_id;
	signreq.data = "";
	signreq.data_len = 0;
	signreq.signature = signature;
	signreq.signature_len = sizeof(signature);
	...
	/* Verify signature matches RFC vector */
	if (memcmp(signature, expected_sig, 64) != 0) {
		TEST_FAIL("signature mismatch with RFC 8032");
		close(fd);
		return (1);
	}
 ...
 */
static int
test_ed25519_rfc8032_vector2(void)
{
	int fd;
	struct kv_import_req importreq;
	struct kv_sign_req signreq;
	...
	TEST_START("Ed25519 RFC 8032 Test Vector 2");
	...
	signreq.key_id = importreq.key_id;
	signreq.data = message;
	signreq.data_len = sizeof(message);
	signreq.signature = signature;
	signreq.signature_len = sizeof(signature);
